...
deploy_path="${DEPLOY_PATH:-/root/wordpress-sites/trovesia-shop}"
deploy_url="${DEPLOY_URL:-https://www.trovesia.com/}"
...
for component in "$@"; do
    case "${component}" in
        theme)
            ...
            component_slugs+=("trovesia")
            local_paths+=("${project_dir}/theme/trovesia")
            required_files+=("style.css")
            ;;
        plugin)
            component_types+=("plugin")
            component_slugs+=("trovesia-plugin")
            local_paths+=("${project_dir}/plugin/trovesia-plugin")
            required_files+=("trovesia-plugin.php")
            ;;
        *)
